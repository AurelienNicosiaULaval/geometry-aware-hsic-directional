...
The R scripts and CSV files used to generate the reported analyses are included with the accompanying replication materials for this manuscript, available at \url{https://github.com/AurelienNicosiaULaval/geometry-aware-hsic-directional} and archived on Zenodo under the concept DOI \url{https://doi.org/10.5281/zenodo.20617102}.  The Noshiro data used here are stored as \texttt{data/noshiro.csv}, a local CSV export of \texttt{ridgetorus::earthquakes}; provenance notes are stored in \texttt{data/noshiro\_source.txt}.
