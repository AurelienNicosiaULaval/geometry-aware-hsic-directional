...
\subsection{Type-I error under independent circular marginals}

Figure~\ref{fig:type1} displays the type-I error estimates under independent circular marginals, and Table~\ref{tab:type1} gives the corresponding numerical summary with Monte Carlo standard errors.  Across all null scenarios and implemented methods, rejection rates range from \(0.028\) to \(0.072\), which is compatible with the nominal level at the reported Monte Carlo resolution.
...
\input{output/final/tables/table1_type1_summary.tex}

\FloatBarrier

\subsection{Circular-circular power}

Figure~\ref{fig:power} summarizes circular-circular rejection rates across the simulated alternatives and effect sizes.  Shift, double-angle, local and symmetric nonlinear alternatives are detected at high rates by the HSIC methods.  The axial and multimodal alternatives are more sensitive to kernel scale, which motivates the separate scale-sensitivity summary below.
...
Table~\ref{tab:selected-power} reports selected rejection rates at \(n=200\) and \(\rho=8\), where the differences among kernel strategies are easiest to see.  For the axial and multimodal alternatives, the multi-scale procedure reaches rejection rate \(1.000\), while the median heuristic gives \(0.124\) and \(0.042\), respectively.  These contrasts are scenario-specific and should not be read as a uniform superiority statement.

\input{output/final/tables/table2_selected_power.tex}

\FloatBarrier

\subsection{Kernel-parameter strategies and sensitivity}

Figure~\ref{fig:strategy} compares the three HSIC tuning strategies across the circular-circular alternatives.  Table~\ref{tab:strategy} gives a compact descriptive average over the same design.  This average depends on the simulation grid and is included only as a summary of the present study.
...
\input{output/final/tables/table3_strategy_comparison.tex}

Figure~\ref{fig:kappa-sensitivity} gives the reduced fixed-kernel sensitivity study.  It isolates the effect of the von Mises concentration for the independent null, axial, multimodal and symmetric nonlinear scenarios at \(n=100\).  The pattern reinforces that local or multi-modal alternatives can require more localized kernels than those selected by a single median heuristic.
...
\subsection{Toroidal demonstration and runtime}
...
\input{output/final/tables/table6_torus_demo.tex}
...
\input{output/final/tables/table5_runtime_benchmark.tex}
\FloatBarrier
...
During the preparation of this work, the author used ChatGPT and Codex to assist with manuscript editing, R code organization, reproducibility checks and repository preparation.  The statistical methods, simulations, interpretation and final manuscript were reviewed and edited by the author.  No generative AI tool was used to create or alter the scientific figures, data or simulation outputs.  The author takes full responsibility for the content of the publication.
